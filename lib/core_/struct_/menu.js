...
const out = fs.openSync('/home/matheus/Documentos/apps_node/clip-cli/lib/core_/clip.log', 'a');
									...
									let lengthComandos = dataJson[i][ambiente].length;
								        ...
								        for (var j = lengthComandos - 1; j >= 0; j--) {
									        ...
									        console.log(`Clip Iniciou seu ambiente com sucesso!!!`);
									         exec(`gnome-terminal --execute bash -c "${__dirname}/watch_log.sh  ; bash" `,(error, stdout, stderr)=>{
									         	console.log('ouvindo saida de logs');
									         });

									        //   exec(`tail -f /home/matheus/Documentos/apps_node/clip-cli/lib/core_/clip.log`,(error, stdout, stderr)=>{});
